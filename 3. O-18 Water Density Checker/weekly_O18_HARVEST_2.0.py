...
class WeightCollector:
    ...
    def save(self):
        ...
        if self.editing_index is None:
            ...
            self.records[self.editing_index] = row
    ...
    def edit_entry(self, event=None):
        ...
        self.editing_index = self.table.index(self.editing_item)

        # Load values back into controls
        self.synth_box.set(values[0])

        self.captured_weight = float(values[1])

        self.weight_display.config(
            text=f"{self.captured_weight:.3f} g"
        )

        self.captured_label.config(
            text=f"Captured: {self.captured_weight:.3f} g"
        )

        self.volume.delete(0, tk.END)
        self.volume.insert(0, values[2])
